...
public class CreateTableGenerator extends AbstractSqlGenerator<CreateTableStatement> {
    ...
    private void handleDefaultValue(CreateTableStatement statement, Database database, String column, StringBuilder buffer) {
        ...
        if (database instanceof HsqlDatabase && defaultValue instanceof SequenceNextValueFunction) {
            // HSQLDB uses "GENERATED BY DEFAULT AS SEQUENCE" for CREATE TABLE
            SequenceNextValueFunction sequenceFunction = (SequenceNextValueFunction) defaultValue;
            ...
            buffer.append("GENERATED BY DEFAULT AS SEQUENCE ").append(sequenceName);
